module.exports = function (pg,connectionString) {
	return function (req, res, next) {
		if(req.body.type===undefined || req.body.gps===undefined || req.body.radius==undefined){
		...
		pg.connect(connectionString,function(err,client,done) {
			...
			client.query("select * from events where gps_x>$1 and gps_x<$2 and gps_y>$3 and gps_y<$4 and type=$5",[gps_x_min, gps_x_max, gps_y_min, gps_y_max, Type],function(err,result){
				done();
				if(err){
				   res.tpl.error=err;
				   return res.status(200).send(res.tpl.error);
